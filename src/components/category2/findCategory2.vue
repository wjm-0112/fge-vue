...
<script>
    export default {
        ...
        mounted(){
            this.$http.get("http://localhost:8081/category2/findCategory2?index=1").then(req=>{
                // 给 data 中的 list 赋值
        ...
        methods:{
            changePage:function (index) {
                this.$http.get("http://localhost:8081/category2/findCategory2?index="+index).then(req=>{
            ...
            findMo:function(){
                ...
                localStorage.setItem("type",type1);
                this.$router.push('findCategory2Mo');
            },
